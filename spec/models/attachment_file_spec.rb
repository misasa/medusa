...
describe AttachmentFile do
  ...
  describe ".path" do
    ...
    describe "attachment_file_path" do
      ...
      context "when set relative url root" do
        let(:attachment_file_id) { 1 }
        before { allow(Rails.application.config).to receive(:relative_url_root) { "/medusa" } }
        it { expect(subject).to eq("/medusa/system/attachment_files/0000/0001/test.jpg") }
      end
    end
  end
