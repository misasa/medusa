...
describe SearchColumn do
  describe "scope" do
    describe "default_scope" do
      let!(:column1) { FactoryGirl.create(:search_column, display_order: 2) }
      let!(:column2) { FactoryGirl.create(:search_column, display_order: 1) }
      subject { SearchColumn.all }
      it { expect(subject).to eq [column2, column1] }
    end

    describe "model_is" do
      let!(:column1) { FactoryGirl.create(:search_column, datum_type: "Specimen") }
      let!(:column2) { FactoryGirl.create(:search_column, datum_type: "Box") }
      subject { SearchColumn.model_is(Specimen) }
      it { expect(subject).to match_array [column1] }
    end

    describe "user_is" do
      let!(:user1) { FactoryGirl.create(:user_foo) }
      let!(:user2) { FactoryGirl.create(:user_baa) }
      let!(:column1) { FactoryGirl.create(:search_column, user: user1) }
      let!(:column2) { FactoryGirl.create(:search_column, user: user2) }
    ...
    describe "master" do
      ...
      let!(:column1) { FactoryGirl.create(:search_column, user_id: user.id) }
      let!(:column2) { FactoryGirl.create(:search_column, user_id: 0) }
      subject { SearchColumn.master }
      it { expect(subject).to match_array [column2] }
    end

    describe "display_expand" do
      let!(:column1) { FactoryGirl.create(:search_column, display_type: 0) }
      let!(:column2) { FactoryGirl.create(:search_column, display_type: 1) }
      let!(:column3) { FactoryGirl.create(:search_column, display_type: 2) }
  ...
  describe "class methods" do
    describe "update_display" do
      let!(:column1) { FactoryGirl.create(:search_column, user_id: user_id, display_order: 1, display_type: 0) }
      let!(:column2) { FactoryGirl.create(:search_column, user_id: user_id, display_order: 2, display_type: 1) }
      let!(:column3) { FactoryGirl.create(:search_column, user_id: user_id, display_order: 3, display_type: 2) }
      let!(:column4) { FactoryGirl.create(:search_column, user_id: 0, display_order: 4, display_type: 0) }
  ...
  describe "instance methods" do
    describe "none?" do
      let!(:column) { FactoryGirl.create(:search_column, display_type: display_type) }
      subject { column.none? }
