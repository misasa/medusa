...
describe "get pmlame.json with autologin" do
  before do
    ...
    create_data
    #login user_1
    get "/records/#{specimen.global_id}/pmlame.json", params: { format: 'json' }
  end
  let(:user_1) { FactoryBot.create(:user, username: "user_1", email: "user_1@example.com") }
  let(:user_2) { FactoryBot.create(:user, username: "user_2", email: "user_2@example.com") }
  let(:create_data) {}
  describe "records list" do
    ...
    let(:attachment_file) { FactoryBot.create(:attachment_file, data_file_name: "file_name", data_content_type: "application/pdf") }
