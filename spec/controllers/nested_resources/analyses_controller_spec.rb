...
describe NestedResources::AnalysesController do
  ...
  describe "POST create" do
    ...
    context "invalidate" do
      ...
      it { expect(parent.analyses.exists?(name: name)).to eq false}
  ...
  describe "PUT update" do
    ...
    context "present child" do
      ...
      it { expect(parent.analyses.exists?(id: child.id)).to eq true}
    end
  ...
  describe "DELETE destory" do
    ...
    context "present child" do
      before { method }
      it { expect(parent.analyses.exists?(id: child.id)).to eq false}
      it { expect(response).to redirect_to request.env["HTTP_REFERER"] }
    end
    context "none child" do
      let(:child_id){0}
