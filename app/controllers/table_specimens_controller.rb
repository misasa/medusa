class TableSpecimensController < ApplicationController
  ...
  def destroy
    @specimen = Specimen.find(params[:id])
    @parent.table_analyses.where(specimen_id: @specimen.id).destroy_all
    @parent.table_specimens.where(specimen_id: @specimen.id).destroy_all
    #@parent.send(params[:association_name]).delete(@specimen)
    respond_with @parent, location: adjust_url_by_requesting_tab(request.referer)
  end
