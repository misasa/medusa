...
CSV.generate do |csv|
  ...
  if @table.with_place
    csv << ["latitude", ""] + @table.specimens.map{|specimen| specimen.place.try!(:latitude_for_csv)} + ["", ""]
    csv << ["longitude", ""] + @table.specimens.map{|specimen| specimen.place.try!(:longitude_for_csv)} + ["", ""]
  end
